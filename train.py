'''
    LSTM-based Deep Learning Model for Question answering
'''
import os
import random
import argparse
...
parser.add_argument('--resume', default='insurance/V2/checkpoints/model_best.tar', type=str, metavar='PATH', help='path saved params')
...
PAD = '<PAD>'
id_to_word, label_to_ans, label_to_ans_text = load_vocabulary('insuranceQA/V2/vocabulary', 'insuranceQA/V2/InsuranceQA.label2answer.token.encoded')
w2i = {w: i for i, w in enumerate(id_to_word.values(), 1)}
w2i[PAD] = 0
vocab_size = len(w2i)
print('vocab_size:', vocab_size)

train_data = load_data('insuranceQA/V2/InsuranceQA.question.anslabel.token.500.pool.solr.train.encoded', id_to_word, label_to_ans_text)
test_data = load_data2('insuranceQA/V2/InsuranceQA.question.anslabel.token.500.pool.solr.test.encoded', id_to_word, label_to_ans_text)
